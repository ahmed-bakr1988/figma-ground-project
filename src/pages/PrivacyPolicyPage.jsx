...
import { getWebPageSchema, getBreadcrumbSchema } from '../config/seoSchema';

const sectionIcons = {
  introduction: Shield,
  infoCollected: Database,
  howWeUse: Eye,
  sharing: Share2,
  security: Lock,
  cookies: Cookie,
  yourRights: UserCheck,
  retention: Clock,
  changes: RefreshCw,
  contact: Phone,
};
...
export default function PrivacyPolicyPage() {
  ...
  const pageUrl = `${companyInfo.urls.website}/privacy-policy`;
  ...
  const schema = getWebPageSchema(
    {
      ...
      title:
        locale === 'ar'
          ? `سياسة الخصوصية | ${companyInfo.name.brand}`
          : `Privacy Policy | ${companyInfo.name.brand}`,
      description:
        locale === 'ar'
          ? 'سياسة الخصوصية لشركة جراوند تك مصر — كيف نجمع معلوماتك ونستخدمها ونحميها.'
          : 'Privacy Policy for Ground Tech Egypt — how we collect, use, and protect your information.',
        ...
        <div className="relative max-w-7xl mx-auto px-6 lg:px-16">
          ...
          >
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-white leading-tight mb-4">
              ...
              <span className="text-accent">{t('privacyPage.hero.titleHighlight')}</span>
            </h1>
                  ...
                  {sectionKeys.map((key) => {
                    ...
                    return (
                      <a
                        key={key}
                        href={`#privacy-${key}`}
                        className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-gray-600 hover:text-primary hover:bg-primary/5 transition-colors group"
                      >
                        <Icon className="w-4 h-4 text-accent flex-shrink-0" />
                        <span>{t(`privacyPage.sections.${key}.title`)}</span>
                      </a>
                    );
                  })}
                </nav>
